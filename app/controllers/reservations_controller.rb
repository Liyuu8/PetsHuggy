class ReservationsController < ApplicationController
  ...
  def create
    ...
    if current_user == @listing.user
      ...
      reservationByme = @listing.reservations.where(user_id: current_user.id)
      
      # 以前、自分自身で選択した日付
      oldSelectedDates = []
      
      # 以前、自分自身で予約した"予約の日付"を配列に入れていく
      reservationByme.each do |reservation|
        oldSelectedDates.push(reservation.start_date)
      end
      
      # 以前の自分で選択した日付の予約を全て消す
      if oldSelectedDates
